...
# Lees de data in
data <- X230620_MISSING_MIGRANTS_PROJECT_DATA_3
...
```{r}
# Eerst, laad de lubridate package
library(lubridate)
...
```{r}
# Zet de waarden om naar numeriek
data_2023$LOG <- as.numeric(gsub(",", ".", data_2023$LOG))
data_2023$LAT <- as.numeric(gsub(",", ".", data_2023$LAT))

# Plot de wereldkaart met de heatmap
ggplot(data_2023, aes(x = LOG, y = LAT)) +
  geom_polygon(data = world_map, aes(x=long, y = lat, group = group), fill = "white", color = "#BBBBBB") +
  geom_density2d_filled(aes(fill = ..level..), alpha = 0.6) +
  labs(title = "Heatmap van incidenten wereldwijd in 2023",
       x = "Lengtegraad", 
       y = "Breedtegraad",
       fill = "Dichtheid van incidenten") +
